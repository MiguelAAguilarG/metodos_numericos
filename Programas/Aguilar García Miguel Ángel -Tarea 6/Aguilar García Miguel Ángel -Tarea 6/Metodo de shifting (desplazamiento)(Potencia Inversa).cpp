...
/*Metodo de shifting (desplazamiento)(Potencia Inversa)*/
...
double sum=0,amold=0,amnew=0,aux=0,auxb=0,auxa[100],auxm[100][100],mi[100][100],minew[100][100],manew[100][100],var=0;double ma[100][100],xold[100][100],xnew[100][100];double ma2[100][100],ma3[100][100],maux3[100][100],tol=0,er1=0,er2=0,max=0,res[100],sumaux3=0,suma3=0,tol2=0,ma3old[100][100],v[100][100],mmaux[100][100],mb[100][100],maux[100][100],mu[100][100],ml[100][100],mad[100][100],mat[100][100];
FILE *f1,*f2,*f3;
...
do
{
      ...
      system("cls");
      fclose(f1);
      printf("Metodo de shifting (desplazamiento)(Potencia Inversa)\n");
...
/*Insertar tolerancia e iteraciones*/
               ...
               scanf("%d",&iter);
               fflush(stdin);
               printf("\nInserte constante (si desea que de el resultado del 'Metodo de la potencia inversa' inserte '0')= ");
               scanf("%lf",&var);
               fflush(stdin);
               printf("\nInserte tolerancia= ");
               scanf("%lf",&tol);
...
/*Metodo de shifting (desplazamiento)***********/
...
/*Multiplicacion de 'var' por 'mi'*/                          
   for(i=1;i<=mm;i++)
   {
   minew[i][1]=var*mi[i][1];          
   } 
/****************/ 

/*Resta de 'A' menos 'var*mi'*/
for(i=1;i<=mm;i++)
{
   for(j=1;j<=nn;j++)
   {          
    manew[i][j]=ma[i][j]-minew[i][j];                
   }
}
/****************/
...
if(k-1>=1)
{
      for(j=1;j<=k-1;j++)
      { 
      sum=sum+((ml[k][j])*(mad[j][1]));
      }
}
  mad[k][1]=xold[k][1]-sum;  
  mad[k][1]=mad[k][1]/ml[k][k];
}  
/****************/
...
for(k;k>=1;k--)
{
      ...
      for(j;j<=mm3;j++)
      { 
      sum=sum+((mu[k][j])*(mat[j][1]));
      }
      
      }
  mat[k][1]=mad[k][1]-sum;  
  mat[k][1]=mat[k][1]/mu[k][k];
}
/****************/
